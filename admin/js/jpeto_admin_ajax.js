...
$(document).ready(function()
{
	$('body').on('click','img.b_adds',function() {
		var objref = $.parseJSON(atob($(this).parents('.admindata').first().data('jpeto')));
	...
	$('body').on('click','img.b_sortu',function() {
		...
		if ($(this).parents('.admindata').first().prev().data('jpeto')) {
			var objrefp = $.parseJSON(atob($(this).parents('.admindata').first().prev().data('jpeto')));
			...
			var objrefp = $.parseJSON(atob($(this).parents('.admindata').first().parent().first().prev().children().first().data('jpeto')));
		...
		if (p1ID!='' && p2ID!='' && !ctrlPressed && !shiftPressed) {
			$.getJSON(jpeto_var_adminpath+'jpeto_admininterface_ajax_gateway.php?'+$.param({
				...
				function(data) {
					if (data['succ']=='1') {
						var objref = $.parseJSON(atob($('#'+data['p1ID']).data('jpeto')));
						var objrefp = $.parseJSON(atob($('#'+data['p2ID']).data('jpeto')));
						...
						objrefp.K = ''+objref.K;
						objref.K = remK;
						$('#'+data['p1ID']).data('jpeto',btoa(JSON.stringify(objref)));
						$('#'+data['p1ID']).attr('data-jpeto',btoa(JSON.stringify(objref)));
						$('#'+data['p2ID']).data('jpeto',btoa(JSON.stringify(objrefp)));
						$('#'+data['p2ID']).attr('data-jpeto',btoa(JSON.stringify(objrefp)));
					}
				}
			);
	...
	$('body').on('click','img.b_sortd',function() {
		...
		if ($(this).parents('.admindata').first().next().data('jpeto')) {
			var objrefn = $.parseJSON(atob($(this).parents('.admindata').first().next().data('jpeto')));
			...
			var objrefn = $.parseJSON(atob($(this).parents('.admindata').first().parent().first().next().children().first().data('jpeto')));
		...
		if (p1ID!='' && p2ID!='' && !ctrlPressed && !shiftPressed) {
			$.getJSON(jpeto_var_adminpath+'jpeto_admininterface_ajax_gateway.php?'+$.param({
				...
				function(data) {
					if (data['succ']=='1') {
						var objref = $.parseJSON(atob($('#'+data['p1ID']).data('jpeto')));
						var objrefn = $.parseJSON(atob($('#'+data['p2ID']).data('jpeto')));
						var butt1 = $('#'+data['p1ID']+' .b_onl').parent().parent().children('td').eq(1).clone();
						var butt2 = $('#'+data['p2ID']+' .b_onl').parent().parent().children('td').eq(1).clone();
						$('#'+data['p1ID']+' .b_onl').parent().parent().children('td').eq(1).replaceWith(butt2);
						$('#'+data['p2ID']+' .b_onl').parent().parent().children('td').eq(1).replaceWith(butt1);
						...
						$('#'+data['p1ID']).data('jpeto',btoa(JSON.stringify(objref)));
						$('#'+data['p1ID']).attr('data-jpeto',btoa(JSON.stringify(objref)));
						$('#'+data['p2ID']).data('jpeto',btoa(JSON.stringify(objrefn)));
						$('#'+data['p2ID']).attr('data-jpeto',btoa(JSON.stringify(objrefn)));
					}
				}
			);
